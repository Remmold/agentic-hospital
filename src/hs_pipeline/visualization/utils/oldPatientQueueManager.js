...
export class PatientQueueManager {
    ...
    spawnPatient(availableCases) {
        ...
        this.loadCaseFile(caseFile).then(caseData => {
            ...
            if (this.activePatient === null) {
                ...
                console.log(`[PatientQueue] âœ… ${patientId} ACTIVE (${spritesheet}) at ${receptionDesk}`);
                ...
                console.log(`[PatientQueue] â³ ${patientId} WAITING (${spritesheet}) at ${waitingChair} - Queue: ${this.waitingPatients.length}/${this.maxWaitingCapacity}`);
            }
        }).catch(error => {
    ...
    handlePatientComplete() {
        if (this.activePatient) {
            ...
            console.log(`[PatientQueue] âœ”ï¸ ${this.activePatient.id} COMPLETED (${elapsed.toFixed(1)}s)`);
        ...
        if (this.waitingPatients.length > 0) {
            ...
            console.log(`[PatientQueue] âœ… ${nextPatient.id} NOW ACTIVE at ${receptionDesk}`);
        }
    }
